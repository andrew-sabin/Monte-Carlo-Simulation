...
const float BEFORE_Y    = 80.f;
const float AFTER_Y     = 20.f;
const float DISTX       = 70.f;
const float RADIUS      = 3.f;

const float BEFORE_YDY  = 5.f;
const float AFTER_YDY   = 1.f;
const float DIST_XDX    = 5.f;
...
// call this if you want your program to use a different random number sequence every time you run it
void TimeOfDaySeed(){
    struct tm y2k = { 0 };
...
int main( int argc, char *argv[ ] ){
        ...
        {
                BeforeY[n] = Ranf(  BEFORE_Y - BEFORE_YDY, BEFORE_Y + BEFORE_YDY );
                AfterY[n]  = Ranf(  AFTER_Y - AFTER_YDY, AFTER_Y + AFTER_YDY );
                DistX[n]   = Ranf(  DISTX - DIST_XDX, DISTX + DIST_XDX );
        }
